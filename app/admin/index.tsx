...
.from(‘event_matches’)
.select(`*, user1:users!event_matches_user1_id_fkey(id, name, email), user2:users!event_matches_user2_id_fkey(id, name, email)`)
.eq(‘event_id’, eventId)
.order(‘level’, { ascending: true })
...
const { data, error } = await supabase
.from(‘appointments’)
.select(`id, user_id, event_id, status, payment_status, created_at, users!inner ( id, name, email, phone, city, country, interested_in, gender, age, age_range_min, age_range_max ), events!inner ( id, name, type, city, date, start_time, event_status )`)
.order(‘created_at’, { ascending: false });
...
supabase
.from(‘appointments’)
.select(`id, user_id, event_id, status, payment_status, created_at, users!inner ( id, name, email, phone, city, country, interested_in, gender, age, age_range_min, age_range_max )`)
.eq(‘event_id’, eventId)
.order(‘created_at’, { ascending: false }),
supabase
.from(‘event_ratings’)
