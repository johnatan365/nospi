...
export default function ProfileScreen() {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      if (!data) {
        ...
        const defaultProfile = { id: user.id, email: user.email || '', name: fullName, date_of_birth: '2000-01-01', age: 24, gender: 'hombre', interested_in: 'ambos', age_range_min: 18, age_range_max: 60, country: 'Colombia', city: 'Medellín', phone: '', profile_photo_url: metadata.avatar_url || null, interests: [], personality_traits: [], compatibility_percentage: 95, notification_preferences: { whatsapp: false, email: true, sms: false, push: true } };
  ...
  const uploadPhoto = async (uri: string) => {
    ...
    try {
      console.log('Uploading profile photo, URI:', uri);
      ...
      const { error: uploadError } = await supabase.storage.from('profile-photos').upload(filePath, uploadData, { contentType: `image/${fileExt}`, cacheControl: '0', upsert: true });
  ...
  const handleMinAgeChange = (value: number) => { const v = Math.round(value); if (v < editAgeRangeMax) setEditAgeRangeMin(v); };
  const handleMaxAgeChange = (value: number) => { const v = Math.round(value); if (v > editAgeRangeMin) setEditAgeRangeMax(v); };
          ...
          <ScrollView style={styles.modalScrollView} contentContainerStyle={styles.modalScrollContent}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.ageSliderSection}>
                ...
                <Slider style={styles.ageSlider} minimumValue={18} maximumValue={59} step={1} value={editAgeRangeMin} onValueChange={handleMinAgeChange} minimumTrackTintColor={ACCENT} maximumTrackTintColor="#E0E0E0" thumbTintColor={ACCENT} />
                ...
                <Slider style={styles.ageSlider} minimumValue={19} maximumValue={60} step={1} value={editAgeRangeMax} onValueChange={handleMaxAgeChange} minimumTrackTintColor={ACCENT} maximumTrackTintColor="#E0E0E0" thumbTintColor={ACCENT} />
              </View>
