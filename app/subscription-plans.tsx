...
export default function SubscriptionPlansScreen() {
  ...
  const loadPSEBanks = useCallback(async () => {
    ...
    try {
      ...
      setPseBanks([
        { code: '1007', name: 'BANCOLOMBIA' }, { code: '1051', name: 'BANCO DAVIVIENDA' },
        { code: '1001', name: 'BANCO DE BOGOTA' }, { code: '1013', name: 'BANCO BBVA COLOMBIA S.A.' },
        { code: '1507', name: 'NEQUI' }, { code: '1551', name: 'DAVIPLATA' },
        { code: '1023', name: 'BANCO DE OCCIDENTE' }, { code: '1032', name: 'BANCO CAJA SOCIAL' },
        { code: '1052', name: 'BANCO AV VILLAS' }, { code: '1002', name: 'BANCO POPULAR' },
      ]);
    } finally {
  ...
  const confirmAppointment = async (transactionId: string, paymentMethod: 'bancolombia' | 'pse' | 'card' | 'nequi' | 'virtual_balance') => {
    try {
      ...
      if (!pendingEventId) { console.log('No pending event ID found'); return; }
  ...
  const handlePayWithVirtualBalance = async () => {
    ...
    try {
      ...
      console.log('[VirtualBalance] Balance deducted');
      const pendingEventId = await AsyncStorage.getItem('pending_event_confirmation');
  ...
  const handleCardPayment = async () => {
    if (!cardNumber || !cardExpiry || !cardCvc || !cardHolder) { showAlert('Error', 'Por favor completa todos los datos de la tarjeta.'); return; }
    if (cardHolder.trim().length < 5) { showAlert('Error', 'El nombre del titular debe tener al menos 5 caracteres.'); return; }
    ...
    try {
      ...
      if (result.status === 'APPROVED') {
        ...
        console.log('[CardPayment] APPROVED');
        const pendingId = await AsyncStorage.getItem('pending_event_confirmation');
        await confirmAppointment(result.transactionId || '', 'card');
        if (pendingId) {
          console.log('[CardPayment] Navigating to event detail with paymentSuccess=true, eventId:', pendingId);
          router.replace({ pathname: '/event-details/[id]', params: { id: pendingId, paymentSuccess: 'true' } });
        } else {
          setShowSuccessModal(true);
        }
      } else if (result.status === 'PENDING') {
        setShowCardForm(false);
        Toast.show({ type: 'info', text1: 'Pago en proceso', text2: 'Tu pago está siendo procesado. Te notificaremos cuando se confirme.', visibilityTime: 5000, position: 'top' });
    ...
    try {
      ...
      const poll = setInterval(async () => {
        ...
        try {
          ...
          if (status === 'APPROVED') {
            clearInterval(poll);
            setProcessingMethod(null); setShowNequiForm(false); setNequiStatus('idle');
            console.log('[NequiPayment] APPROVED');
            const pendingId = await AsyncStorage.getItem('pending_event_confirmation');
            ...
            clearInterval(poll); setProcessingMethod(null); setNequiStatus('idle');
  ...
  const handleBancolombiaPayment = async () => {
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/functions/v1/wompi-bancolombia-payment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` },
        body: JSON.stringify({ acceptanceToken, personalDataToken, amountCOP: priceCOP, userEmail: userProfile?.email || currentUser.email || '', userId: currentUser.id, eventId: pendingEventId, redirectUrl: bancolombiaRedirectUrl }),
      });
      const result = await response.json();
      ...
      setProcessingMethod(null);
      Toast.show({ type: 'info', text1: 'Completa tu pago en Bancolombia', text2: 'Cuando termines, regresa a esta pantalla. Tu pago se verificará automáticamente.', visibilityTime: 8000, position: 'top', topOffset: 60 });
      return;
    } catch (error: any) {
  ...
  const handleTestPayment = async () => {
    ...
    try {
      const pendingEventId = await AsyncStorage.getItem('pending_event_confirmation');
      if (!pendingEventId) { showAlert('Test', 'No hay evento pendiente. Ve a un evento y presiona Confirmar primero.'); return; }
      const { data: { session } } = await supabase.auth.getSession();
      const userId = session?.user?.id;
      if (!userId) { showAlert('Test', 'No hay sesión activa.'); return; }
      ...
      if (existing) {
        ...
        const { error: updateError } = await supabase.from('appointments').update({ status: 'confirmada', payment_status: 'completed' }).eq('id', existing.id);
        if (updateError) { console.error('[TEST] Update error:', updateError.message); showAlert('Test Error', updateError.message); return; }
      } else {
        console.log('[TEST] No existing appointment, inserting new one');
        const { error: insertError } = await supabase.from('appointments').insert({ user_id: userId, event_id: pendingEventId, status: 'confirmada', payment_status: 'completed' });
        if (insertError) { console.error('[TEST] Insert error:', insertError.message); showAlert('Test Error', insertError.message); return; }
      }
      await AsyncStorage.removeItem('pending_event_confirmation');
  ...
  const handlePSEPayment = async () => {
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/functions/v1/wompi-pse-payment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` },
        body: JSON.stringify({ acceptanceToken, personalDataToken, amountCOP: priceCOP, userEmail: pseEmail, userId: currentUser.id, eventId: pendingEventId, userFullName: userProfile?.name || currentUser.email || '', userPhone: cleanPhone, userLegalId: cleanLegalId, userLegalIdType: pseLegalIdType, financialInstitutionCode: pseBankCode, redirectUrl: pseRedirectUrl }),
      });
      const data = await response.json();
      if (!response.ok) { console.error('PSE payment error:', data); throw new Error(data.error || 'Error al crear pago PSE'); }
      ...
      if (Platform.OS === 'web') {
        ...
        setProcessingMethod(null); setShowPSEForm(false);
        startWebPolling(data.transactionId, 'pse');
        return;
      }
      await Linking.openURL(data.redirectUrl);
      setProcessingMethod(null); setShowPSEForm(false);
      Toast.show({ type: 'info', text1: 'Completa tu pago en tu banco', text2: 'Cuando termines, regresa a esta pantalla. Tu pago se verificará automáticamente.', visibilityTime: 8000, position: 'top', topOffset: 60 });
      return;
    } catch (error: any) {
  ...
  const priceText = `$${priceCOP.toLocaleString('es-CO')} COP`;
  const priceAmountText = `$${priceCOP.toLocaleString('es-CO')}`;
  const balanceText = `Disponible: $${virtualBalance.toLocaleString('es-CO')} COP`;

  if (showCardForm) {
    return (
      <SafeAreaView style={styles.formContainer}>
        <Stack.Screen options={{ headerShown: true, title: 'Pagar con Tarjeta', headerLeft: () => (
          <TouchableOpacity onPress={() => { console.log('User cancelled card form'); setShowCardForm(false); }} style={{ paddingHorizontal: 16 }}>
            <Text style={{ color: ACCENT, fontSize: 16 }}>Cancelar</Text>
          </TouchableOpacity>
        )}} />
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={styles.formContent} keyboardShouldPersistTaps="handled">
            <View style={styles.formCard}>
              ...
              <Text style={styles.formAmount}>{priceText}</Text>
              ...
              <View style={[styles.payBtnWrapper, isProcessing('card') && styles.payBtnDisabled]}>
                <LinearGradient colors={['#1a0010', '#880E4F', '#AD1457']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.payBtnGradient}>
                  <TouchableOpacity style={styles.payBtnInner} onPress={() => { Keyboard.dismiss(); handleCardPayment(); }} disabled={isProcessing('card')} activeOpacity={0.7}>
                    {isProcessing('card') ? <ActivityIndicator color="#fff" /> : <Text style={styles.payBtnText}>{`Pagar ${priceText}`}</Text>}
                  </TouchableOpacity>
                </LinearGradient>
              </View>
    ...
    return (
      <SafeAreaView style={styles.formContainer}>
        <Stack.Screen options={{ headerShown: true, title: 'Pagar con Nequi', headerLeft: () => (
          <TouchableOpacity onPress={() => { console.log('User cancelled Nequi form'); setShowNequiForm(false); setNequiStatus('idle'); setProcessingMethod(null); }} style={{ paddingHorizontal: 16 }}>
            <Text style={{ color: ACCENT, fontSize: 16 }}>Cancelar</Text>
          </TouchableOpacity>
        )}} />
        <ScrollView contentContainerStyle={styles.formContent} keyboardShouldPersistTaps="handled">
          <View style={styles.formCard}>
            {nequiStatus === 'idle' ? (
              <>
                ...
                <View style={[styles.payBtnWrapper, isProcessing('nequi') && styles.payBtnDisabled]}>
                  <LinearGradient colors={['#1a0010', '#880E4F', '#AD1457']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.payBtnGradient}>
                    <TouchableOpacity style={styles.payBtnInner} onPress={handleNequiPayment} disabled={isProcessing('nequi')} activeOpacity={0.7}>
              ...
              <View style={styles.waitingContainer}>
                ...
                <Text style={styles.waitingDesc}>Aprueba el pago de <Text style={{ fontWeight: 'bold' }}>{priceText}</Text> desde tu app Nequi.</Text>
    ...
    return (
      <SafeAreaView style={styles.formContainer}>
        <Stack.Screen options={{ headerShown: true, title: 'Pagar con PSE', headerLeft: () => (
          <TouchableOpacity onPress={() => { console.log('User cancelled PSE form'); setShowPSEForm(false); setProcessingMethod(null); }} style={{ paddingHorizontal: 16 }}>
            <Text style={{ color: ACCENT, fontSize: 16 }}>Cancelar</Text>
          </TouchableOpacity>
        )}} />
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={styles.formContent} keyboardShouldPersistTaps="handled">
            <View style={styles.formCard}>
              ...
              <TouchableOpacity style={[styles.input, { justifyContent: 'center' }]} onPress={() => !loadingBanks && setShowBankPicker(true)}>
                {loadingBanks
                  ? <ActivityIndicator size="small" color={ACCENT} />
                  : <Text style={{ fontSize: 16, color: pseBankName ? '#111' : '#aaa' }}>{pseBankName || 'Selecciona tu banco'}</Text>
                }
              </TouchableOpacity>
              {showBankPicker && (
                <View style={styles.bankPickerContainer}>
                  <ScrollView style={{ maxHeight: 220 }} nestedScrollEnabled>
                    {pseBanks.map((bank) => (
                      <TouchableOpacity key={bank.code} style={styles.bankOption} onPress={() => { setPseBankCode(bank.code); setPseBankName(bank.name); setShowBankPicker(false); }}>
                        <Text style={[styles.bankOptionText, pseBankCode === bank.code && { color: ACCENT, fontWeight: '700' }]}>{bank.name}</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                </View>
              ...
              <TextInput style={styles.input} placeholder="tucorreo@gmail.com" value={pseEmail} onChangeText={setPseEmail} keyboardType="email-address" autoCapitalize="none" returnKeyType="next" />
              <Text style={styles.inputLabel}>Tipo de documento</Text>
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 4 }}>
                {['CC', 'CE', 'NIT', 'PP'].map((tipo) => (
                  <TouchableOpacity key={tipo} style={[styles.installmentBtn, pseLegalIdType === tipo && styles.installmentBtnSelected]} onPress={() => setPseLegalIdType(tipo)}>
                    <Text style={[styles.installmentText, pseLegalIdType === tipo && styles.installmentTextSelected]}>{tipo}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <Text style={styles.inputLabel}>Número de documento</Text>
              <TextInput style={styles.input} placeholder="123456789" value={pseLegalId} onChangeText={(t) => setPseLegalId(t.replace(/\D/g, '').slice(0, 15))} keyboardType="numeric" maxLength={15} returnKeyType="next" />
              <Text style={styles.inputLabel}>Celular</Text>
              <TextInput style={styles.input} placeholder="3001234567" value={psePhone} onChangeText={(t) => setPsePhone(t.replace(/\D/g, '').slice(0, 10))} keyboardType="phone-pad" maxLength={10} returnKeyType="done" />
              <View style={[styles.payBtnWrapper, isProcessing('pse') && styles.payBtnDisabled]}>
                <LinearGradient colors={['#1a0010', '#880E4F', '#AD1457']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.payBtnGradient}>
                  <TouchableOpacity style={styles.payBtnInner} onPress={() => { Keyboard.dismiss(); handlePSEPayment(); }} disabled={isProcessing('pse')} activeOpacity={0.7}>
                    {isProcessing('pse') ? <ActivityIndicator color="#fff" /> : <Text style={styles.payBtnText}>{`Pagar ${priceText}`}</Text>}
                  </TouchableOpacity>
                </LinearGradient>
              </View>
              <Text style={styles.secureNote}>🔒 Pago seguro procesado por Wompi</Text>
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
        ...
        <Text style={styles.subtitle}>{`Confirma tu asistencia pagando ${priceText}`}</Text>
          ...
          <TouchableOpacity style={styles.paymentBtn} onPress={handlePayWithVirtualBalance} disabled={processing} activeOpacity={0.85}>
            <View style={styles.btnInner}>
              ...
              <View style={styles.btnTextWrap}>
                ...
                <Text style={styles.btnSub}>{balanceText}</Text>
              </View>
              {isProcessing('virtual') ? <ActivityIndicator color={HEADING} size="small" /> : <Text style={styles.btnArrow}>›</Text>}
            </View>
          </TouchableOpacity>
        )}

        <TouchableOpacity style={styles.paymentBtn} onPress={() => { console.log('User selected Tarjeta de Crédito'); setShowCardForm(true); }} disabled={processing} activeOpacity={0.85}>
          <View style={styles.btnInner}>
            <Text style={styles.btnIcon}>💳</Text>
            <View style={styles.btnTextWrap}>
        ...
        <TouchableOpacity style={styles.paymentBtn} onPress={() => { console.log('User selected Nequi'); setShowNequiForm(true); }} disabled={processing} activeOpacity={0.85}>
          <View style={styles.btnInner}>
        ...
        <TouchableOpacity style={styles.paymentBtn} onPress={handleBancolombiaPayment} disabled={processing} activeOpacity={0.85}>
          <View style={styles.btnInner}>
            ...
            {isProcessing('bancolombia') ? <ActivityIndicator color={HEADING} size="small" /> : <Text style={styles.btnArrow}>›</Text>}
          </View>
        </TouchableOpacity>

        <TouchableOpacity style={styles.paymentBtn} onPress={() => { console.log('User selected PSE'); setShowPSEForm(true); loadPSEBanks(); }} disabled={processing} activeOpacity={0.85}>
          <View style={styles.btnInner}>
            ...
            {isProcessing('pse') ? <ActivityIndicator color={HEADING} size="small" /> : <Text style={styles.btnArrow}>›</Text>}
          </View>
        </TouchableOpacity>

        <TouchableOpacity style={testPaymentStyles.btn} onPress={handleTestPayment} activeOpacity={0.7}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.successButtonWrapper}>
              <LinearGradient colors={['#1a0010', '#880E4F', '#AD1457']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.successButtonGradient}>
                <TouchableOpacity style={styles.successButtonInner} onPress={() => { console.log('Success modal dismissed'); setShowSuccessModal(false); router.replace('/(tabs)/appointments'); }}>
...
const testPaymentStyles = StyleSheet.create({
  btn: { backgroundColor: '#F3F4F6', borderRadius: 12, paddingVertical: 14, alignItems: 'center', marginBottom: 16, borderWidth: 1, borderColor: '#E5E7EB' },
  btnText: { color: MUTED, fontSize: 14, fontWeight: '600' },
});
...
const styles = StyleSheet.create({
  ...
  formCard: { backgroundColor: '#fff', borderRadius: 20, padding: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8, elevation: 4, borderWidth: 1, borderColor: '#E5E7EB' },
  formTitle: { fontSize: 22, fontWeight: '800', color: HEADING, marginBottom: 4 },
  formAmount: { fontSize: 28, fontWeight: 'bold', color: HEADING, marginBottom: 20 },
  inputLabel: { fontSize: 14, fontWeight: '600', color: BODY, marginBottom: 6, marginTop: 12 },
  ...
  installmentBtnSelected: { borderColor: ACCENT, backgroundColor: '#F8BBD0' },
  installmentText: { fontSize: 13, color: MUTED, fontWeight: '600' },
  installmentTextSelected: { color: HEADING },
  payBtnWrapper: { borderRadius: 14, overflow: 'hidden', marginTop: 24 },
  payBtnGradient: { borderRadius: 14 },
  payBtnInner: { paddingVertical: 18, alignItems: 'center' },
  ...
  nequiDescription: { fontSize: 14, color: MUTED, lineHeight: 22, marginBottom: 8 },
  waitingContainer: { alignItems: 'center', padding: 16 },
  waitingTitle: { fontSize: 22, fontWeight: '800', color: HEADING, marginBottom: 12 },
  waitingDesc: { fontSize: 15, color: MUTED, textAlign: 'center', lineHeight: 24 },
  waitingHint: { fontSize: 13, color: '#999', marginTop: 12 },
  title: { fontSize: 30, fontWeight: 'bold', color: HEADING, marginBottom: 8 },
  subtitle: { fontSize: 15, color: MUTED, marginBottom: 24, lineHeight: 22 },
  priceCard: { backgroundColor: '#F9FAFB', borderRadius: 20, padding: 24, marginBottom: 20, alignItems: 'center', borderWidth: 1, borderColor: '#E5E7EB' },
  priceLabel: { fontSize: 15, color: MUTED, marginBottom: 6 },
  priceAmount: { fontSize: 44, fontWeight: 'bold', color: HEADING },
  priceAmountCOP: { fontSize: 18, fontWeight: '600', color: ACCENT, marginTop: 4 },
  benefitsCard: { backgroundColor: '#F9FAFB', borderRadius: 20, paddingVertical: 8, paddingHorizontal: 20, marginBottom: 28, borderWidth: 1, borderColor: '#E5E7EB' },
  benefitRow: { flexDirection: 'row', alignItems: 'flex-start', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
  benefitIcon: { fontSize: 24, marginRight: 14, marginTop: 2 },
  benefitTextWrap: { flex: 1 },
  benefitTitle: { fontSize: 15, fontWeight: '700', color: HEADING, marginBottom: 2 },
  benefitDesc: { fontSize: 13, color: MUTED, lineHeight: 18 },
  sectionTitle: { fontSize: 18, fontWeight: '700', color: HEADING, marginBottom: 16 },
  paymentBtn: { backgroundColor: '#FFFFFF', borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: '#E5E7EB', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  btnInner: { flexDirection: 'row', alignItems: 'center', padding: 16 },
  btnIcon: { fontSize: 28, marginRight: 14 },
  btnLogo: { width: 40, height: 40, marginRight: 14 },
  btnTextWrap: { flex: 1 },
  btnTitle: { fontSize: 16, fontWeight: '700', color: HEADING },
  btnSub: { fontSize: 13, color: MUTED, marginTop: 2 },
  btnArrow: { fontSize: 24, color: MUTED, fontWeight: '300' },
  secureFooter: { fontSize: 13, color: MUTED, textAlign: 'center', marginTop: 8, marginBottom: 16 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 24 },
  modalContent: { backgroundColor: '#fff', borderRadius: 20, padding: 32, alignItems: 'center', width: '100%', maxWidth: 360 },
  successIcon: { fontSize: 56, marginBottom: 16 },
  successTitle: { fontSize: 24, fontWeight: '800', color: HEADING, marginBottom: 8, textAlign: 'center' },
  successMessage: { fontSize: 16, color: MUTED, textAlign: 'center', marginBottom: 24, lineHeight: 22 },
  successButtonWrapper: { borderRadius: 14, overflow: 'hidden', width: '100%' },
  successButtonGradient: { borderRadius: 14 },
  successButtonInner: { paddingVertical: 14, paddingHorizontal: 40, alignItems: 'center' },
  successButtonText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  bankPickerContainer: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', marginBottom: 8 },
  bankOption: { paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
  bankOptionText: { fontSize: 15, color: BODY },
  BODY: { color: BODY },
});
